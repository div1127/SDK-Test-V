...
describe('RegisterForm', function(){
    ...
    var Form = {
        ...
        expandstateList:function(){
         return ST.component('comboboxfield[name="state"] expandtrigger[type="expand"]');   
        },
    ...
    describe('Fields editing', function() {
        ...
        it('Pickerfield', function() {
        Form.statePicker().type('New York')
        Form.statePickerList().wait(2000).click().and(function(){
        Form.statePicker().and(function() {
    ...
    describe('Required fields', function() {
        ...
        for(var i=0; i<required.length; i++){
            it('tooltip is displayed on' +fieldLabels[required[i]], function() {
                
                
                Lib.Forms.tooltipError('textfield[_label='+fieldLabels[required[incT]]+']', 'This field is required');
                incT++;
            });
        }    
    ...
    describe('State comboBox selection', function() {
        it('select a state', function() {
    
          Form.expandstateList().click();
            ST.component('boundlist')
                .visible()
                .and(function(l) {
                    expect(l._itemCount).toBe(51);    
                }).down('simplelistitem[$datasetIndex=0]')
                    .click()
                    .hidden();
            Form.statePicker().focus().value('AL');
            Form.field(7).and(function() {
                expect(this.future.cmp._inputValue).toBe('Alabama');
            });
            Form.clear(7, true);
        });
        it('auto suggestion', function() {
            Form.statePicker().setValue('')
                .type('Ha')
            ST.component('boundlist')
                .visible()
                .and(function(l) {

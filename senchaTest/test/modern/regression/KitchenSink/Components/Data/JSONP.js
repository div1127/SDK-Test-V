...
describe('JSONP example', function () {
    ...
    describe('JSONP Data', function () {
        ...
        it('are loaded after clicking on button Load using JSON-P - EXTJS-26238', function () {
            ...
            var day = correctDay < 10 ? '0' + correctDay : correctDay;
	        var ieEdgeText = day + '-' + month + '-' + time.getFullYear();
            ...
            Cmp.buttonByText('Load using JSON-P')
	        	...
	        	if((ST.browser.is('IE') || ST.browser.is('Edge'))) {
                     ST.wait(function () {
                        return Cmp.getPanelText().replace(/\s|\u200e/g,'').indexOf(ieEdgeText) >= 0;
                     })
                     .and(function () {
                        expect(Cmp.getPanelText().replace(/\s|\u200e/g,'')).toContain(ieEdgeText);
                    });
                }
                else {
                      ST.wait(function () {

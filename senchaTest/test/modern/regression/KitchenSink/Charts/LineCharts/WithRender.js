...
describe("With Render", function() {
    ...
    function valueColor(c, isPositive){
        ...
        for(var i = 0; i < c.getStore().data.items.length; i++){
            ...
            if(current > 0){
                ...
                instance = c.getSeries()[0].getSurface().getItems()[0].boundMarkers.markers.instances[i];
                strokeStyle = '#6495ed';
                fillStyle = '#f0f8ff';
    ...
    describe(desc, function () {
        ...
        it('Part from point to point on its right side, which has lower y value, has red color', function () {
            ST.component('chart')
                .and(function(c){
                    valueColor(c, false);
                });
        ...
        it('Part from point to point on its right side, which has higher y value, has blue color', function () {
            ST.component('chart')
                .and(function(c){
                    valueColor(c, true);
                });

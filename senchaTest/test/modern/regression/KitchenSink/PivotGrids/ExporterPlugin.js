...
describe('PivotGrids Exporter Plugin', function(){
    ...
    var Grid = {
        ...
        createItForScrollToRecordRow: function(index, down){
            it('Should be scrolled to row[' + index + ']', function(){
                ...
                Grid.getGrid()
                    .wait(function(){
                        var scrollTop = this.cmp.getScrollable().getPosition().y;
                        if(down){
                            return scrollTop === 0;
                        } else {
                            return scrollTop !== 0;
                ...
                Grid.scrollToRecord(index)
                    ...
                    .and(function(){
                        var scrollTop = this.future.cmp.getScrollable().getPosition().y;
            ...
            it('Reveal to Row[' + row + '] and cell[' + cell + '] from scrollTop=' + setTop + ' and scrollLeft=' + setLeft, function(){
                ...
                Grid.getGrid()
                    .and(function(){
                        ...
                        var scrollTop = this.future.cmp.getScrollable().getPosition().y;
                        var scrollLeft = this.future.cmp.getScrollable().getPosition().x;
        ...
        setScrollToTopAndLeft: function(top, left){
            this.getGrid().visible().and(function(){
                this.future.cmp.getScrollable().scrollTo(left, top);
            });
        }
    };
    beforeAll(function(){
    ...
    describe('Default display UI and correctly loaded', function(){
        ...
        // TODO: Fails on Win 10 / IE 11
        xit('Should be collapsed all of Rows items', function(){
            Grid.isCountRows(defaultCountRow);
        });
        // TODO: Fails on Win 10 / IE 11
        xit('Should be collapsed all of Columns items', function(){
            Grid.checkCountParentColumnsWhenCountColumnsIs(defaultCountColumn)
        });
    ...
    describe('Expandable/Collapsible', function(){
        ...
        xdescribe('Expanding and collapsing columns', function(){
            /**
             * Here create "it"s for expanding and collapsing collumn
             **/
            for (var ey = 2012; ey <= d.getFullYear(); ey++)
            {
                Grid.createItForExpandYearColumn(ey);
            }
            it('Expanded/Collapsed all year columns', function(){
                Grid.isCountColumns(defaultCountColumn);
                Grid.expandAllColumns();
                Grid.waitOnCountColumnsGreaterThan(countColumn);
                Grid.isCountColumnsGreaterThan(countColumn);
                Grid.collapseAllColumns(true);
                Grid.checkCountParentColumnsWhenCountColumnsIs(defaultCountColumn);
            });
        });
    ...
    describe('Pivotgrid is scrollable', function(){
        ...
        xdescribe('Scroll to the item with using reveal()', function(){
            /**
             * Here create "it"s for reveal (scroll)
             * But these tests didn't working for ExtJS 6.5 due to EXTJS-23747
             **/
            Grid.createItForReveal(10, 10, 0, 0);
            Grid.createItForReveal(10, 10, countRow * 33, countColumn * 120);
            Grid.createItForReveal(0, 0, countRow * 33, countColumn * 120);
            Grid.createItForReveal(countRow - 1, countColumn - 1, 0, 0);
        });
    ...
    xdescribe('Configurator window', function(){
        // Tests for Configurator plugin are at Configurator.js file 
        it('Open & Close Configurator', function(){
            Grid.getButton(textButtons[1]).click();
            ST.wait(function(){
                return Ext.first(gridPrefix + 'pivotconfigpanel[_modal=true]{isVisible()} ');
            }).and(function(){
                expect(Ext.first(gridPrefix + 'pivotconfigpanel[_modal=true]{isVisible()} ')).toBeDefined();
            });
            ST.button(gridPrefix + 'pivotconfigpanel[_modal=true]{isVisible()} button[text=Done]{isVisible()}').click();
            ST.wait(function(){
                return !Ext.first(gridPrefix + 'pivotconfigpanel[_modal=true]{isVisible()} ');
            }).and(function(){
                expect(Ext.first(gridPrefix + 'pivotconfigpanel[_modal=true]{isVisible()} ')).toBeNull();
            });
        });
    });

        ...
        */
        tooltipError: function(element, error){
            ...
            var offsetHeight = Ext.first(element).el.dom.clientHeight-20;            
                if(Lib.isDesktop) {
                    ST.component(element).type({
                        key: 'TAB'
                    });    
                    ST.play([
                        { type: "mouseover", target: element,x:10, y:offsetHeight},
                        { type: "mouseover", target: element,x:10, y:offsetHeight},
                        { type: "mouseover", target: element,x:10, y:offsetHeight},
                        { type: "mouseover", target: element,x:10, y:offsetHeight},
                        { type: "mouseover", target: element,x:10, y:offsetHeight},
                        { type: "mouseover", target: element,x:10, y:40},
                    ]);
                }

            ST.component('#ext-global-tooltip')
                .visible()
                .and(function(tip) {
                ...
                ST.component(chart.replace('-main', ''))
                    .and(function(el){
                        for(var i = 0; i <el.getSeries()[0].getSurface().getItems().length; i++){
                            ...
                            else if(el.getSeries()[0].getSurface().getItems()[i].type === 'lineSeries' 
                            || el.getSeries()[0].getSurface().getItems()[i].type === 'radar' 
                            || el.getSeries()[0].getSurface().getItems()[i].type === 'scatterSeries'){
                                instances = el.getSeries()[0].getSurface().getItems()[i].boundMarkers.markers.instances;
                                if (highlihtedDimension === 0) {
                                    break;
                                }
                                highlihtedDimension--;
...
function testMenuItemProcedure(panelText, button1, messageBox){
    ...
    ST.component(button1).hasCls('x-menu-item-active').wait(5000);
